...
public class DataParser {
	...
	static class DataStopTime {
		...
		public Object getArray() {
			//return null;
			String[] result = {trip_id, stop_id.toString(),arrival_time,departure_time,stop_sequence.toString()};
			return result;	
		}
	 ...
	 * @return map <trip_id, list of all DataStopTimes with this id>
	 */
	static Map<Float, List<DataStopTime>> getStopTimesByStopId(String fileName) {
		
		Map<Float, List<DataStopTime>> result = new HashMap<Float, List<DataStopTime>>();
		File input = new File(fileName);
		Scanner sc;
		try {
			sc = new Scanner(input);
			...
			while (sc.hasNextLine()) {
				...
				result.put(dataEntry.stop_id, stopTimes);
			}
			sc.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
